...
async def admin_stats(message: Message):
    ...
    try:
        ...
        stats_text = (
            ...
            f"<b><tg-emoji emoji-id='{EMOJI_CALENDAR}'>📅</tg-emoji> Продажи:</b>\n"
...
async def country_selected(callback: CallbackQuery):
    ...
    try:
        ...
        if callback.from_user.id in ADMIN_IDS and callback.from_user.id in pending_sales and pending_sales[callback.from_user.id].get("action") == "set_price_select":
            pending_sales[callback.from_user.id] = {
                "action": "set_price",
                "country_id": country_id
            }
            await callback.message.edit_text(
                f"<b><tg-emoji emoji-id='{EMOJI_PENCIL}'>✏️</tg-emoji> Введите новую цену для {country.name}:</b>",
                parse_mode=ParseMode.HTML
            )
...
async def my_sales(callback: CallbackQuery):
    ...
    try:
        ...
        sales_text = f"<b><tg-emoji emoji-id='{EMOJI_BOX}'>📦</tg-emoji> История продаж:</b>\n\n"
        for sale in sales:
            status_map = {
            ...
            sales_text += (
                f"<b>Дата:</b> {sale.created_at.strftime('%d.%m.%Y %H:%M')}\n"
    ...
    if sale_data["step"] == "phone":
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Продажа #{sale.id} создана пользователем {user_id}")

...
# -------------------- Сопоставление стран с phone country codes --------------------
COUNTRY_CODE_MAP = {
    "США": ["US"],
    "Россия": ["RU"],
    "Беларусь": ["BY"],
...
# -------------------- Функция определения страны по номеру --------------------
def get_country_from_phone(phone_number: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"Ошибка определения страны для номера {phone_number}: {e}")
        return None
...
async def withdraw_method(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"<b><tg-emoji emoji-id='{EMOJI_SEND_MONEY}'>💸</tg-emoji> Для вывода средств свяжитесь с администратором: @v3estnikov</b>",
        parse_mode=ParseMode.HTML,
        reply_markup=get_withdraw_keyboard()
    )
    await callback.answer()
...
async def handle_sell_process(message: Message):
    ...
    if sale_data["step"] == "phone":
        ...
        if not detected_country:
            await message.answer(
                f"<b><tg-emoji emoji-id='{EMOJI_CROSS}'>❌</tg-emoji> Не удалось определить страну по номеру телефона. Проверьте номер.</b>",
                parse_mode=ParseMode.HTML
            )
            return

        # Получаем выбранную страну из базы
        db = SessionLocal()
        try:
            ...
            if not selected_country:
                await message.answer(
                    f"<b><tg-emoji emoji-id='{EMOJI_CROSS}'>❌</tg-emoji> Ошибка: страна не найдена.</b>",
                    parse_mode=ParseMode.HTML
                )
            ...
            if detected_country != selected_country.name:
                await message.answer(
                    f"<b><tg-emoji emoji-id='{EMOJI_CROSS}'>❌</tg-emoji> Номер телефона не соответствует выбранной стране!</b>\n\n"
                    f"<b>Выбрана:</b> {selected_country.name}\n"
                    f"<b>Определена по номеру:</b> {detected_country}\n\n"
                    f"Пожалуйста, выберите правильную страну или введите корректный номер.",
                    parse_mode=ParseMode.HTML,
                    reply_markup=get_countries_keyboard()
                )
                del pending_sales[user_id]
            ...
            logger.info(f"Проверка страны пройдена: {phone} -> {detected_country} (выбрана: {selected_country.name})")

        finally:
            db.close()

        # Если всё ок - отправляем код
        ...
        try:
            ...
            await message.answer(
                f"<b><tg-emoji emoji-id='{EMOJI_CHECK}'>✅</tg-emoji> Страна подтверждена: {detected_country}</b>\n"
